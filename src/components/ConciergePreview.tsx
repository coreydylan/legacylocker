...
interface ConciergePreviewProps {
  samples: StorySample[];
  className?: string;
  onSampleSelect?: (sample: StorySample) => void;
}

export const ConciergePreview: React.FC<ConciergePreviewProps> = ({
  samples,
  className,
  onSampleSelect,
}) => {
    ...
    <>
          ...
          <div className="space-y-4 w-[500px]">
            ...
            <p className="flex items-start">
              ...
              <button
                ...
                className="group text-left px-2 py-1 rounded-sm transition-colors text-legacy-slate text-[clamp(20px,2vw,26px)] leading-relaxed flex items-center bg-orange-500/10 hover:bg-orange-500/20"
              >
      ...
      <div className="md:hidden w-full">
        ...
        <div className="mt-8 px-6 space-y-6">
          ...
          <button
            onClick={handleExploreCTA}
            className="w-full flex items-center justify-center space-x-2 py-3 text-sm font-medium text-legacy-slate bg-orange-500/10 hover:bg-orange-500/20 rounded-lg transition-colors"
          >
            <span>Connect with a Concierge</span>
